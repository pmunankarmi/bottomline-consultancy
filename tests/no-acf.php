...
$slug=$argv[1] ?? 'about'; {
 $page_post=get_page_by_path($slug);$GLOBALS['wp_query']=new WP_Query(['page_id'=>$page_post->ID]);ob_start();include get_template_directory().'/page-'.$slug.'.php';$html=ob_get_clean();if(!str_contains($html,'</html>'))throw new RuntimeException('Incomplete '.$slug);echo 'PASS no ACF: '.$slug.PHP_EOL;
}
