<?php
/**
 * The growth metrics illustration for a hero slide.
 *
 * @package Bottomline
 */
...
?>
<div class="hero-dash growth-grid" aria-label="<?php echo esc_attr( $dashboard['aria_label'] ?? '' ); ?>">
	<?php
	foreach ( bl_rows( $dashboard['hero_dash_growth_grid_items'] ?? '' ) as $metric ) :
		if ( ! bl_has_content( $metric ) ) {
			continue;
		}
		$svg_id = wp_unique_id( 'bl-chart-' );
		if ( ( $metric['layout'] ?? '1' ) === '1' ) :
			?>
			<div class="gg-tile">
				<div class="gg-head">
					<span class="gg-label"><?php echo bl_text( $metric['gg_label'] ?? '' ); ?></span>
					<span class="gg-delta up">
						▲
						<span class="num" data-target="<?php echo esc_attr( $metric['data_target'] ?? '' ); ?>"><?php echo esc_html( bl_number( $metric['data_target'] ?? '' ) ); ?></span>
						%
					</span>
				</div>
				<div class="gg-val">
				<?php echo bl_text( $metric['gg_val'] ?? '' ); ?>
					<span class="num" data-target="<?php echo esc_attr( $metric['data_target_2'] ?? '' ); ?>"><?php echo esc_html( bl_number( $metric['data_target_2'] ?? '' ) ); ?></span>
					<span class="gg-unit"><?php echo bl_text( $metric['gg_unit'] ?? '' ); ?></span>
				</div>
				<svg class="gg-chart" viewBox="0 0 160 46" preserveAspectRatio="none">
					<defs>
						<linearGradient id="<?php echo esc_attr( $svg_id . 'ggf1' ); ?>" x1="0" y1="0" x2="0" y2="1">
							<stop offset="0%" stop-color="#4dd499" stop-opacity="0.45"></stop>
							<stop offset="100%" stop-color="#4dd499" stop-opacity="0"></stop>
						</linearGradient>
					</defs>
			...
			<div class="gg-tile">
				<div class="gg-head">
					...
					<span class="gg-delta up">
						▲
						<span class="num" data-target="<?php echo esc_attr( $metric['data_target'] ?? '' ); ?>"><?php echo esc_html( bl_number( $metric['data_target'] ?? '' ) ); ?></span>
						<?php echo bl_text( $metric['gg_delta_up'] ?? '' ); ?>
					</span>
				</div>
				<div class="gg-val">
					<span class="num" data-target="<?php echo esc_attr( $metric['data_target_2'] ?? '' ); ?>"><?php echo esc_html( bl_number( $metric['data_target_2'] ?? '' ) ); ?></span>
					<span class="gg-unit">%</span>
				</div>
				<svg class="gg-chart" viewBox="0 0 160 46" preserveAspectRatio="none">
					<defs>
						<linearGradient id="<?php echo esc_attr( $svg_id . 'ggf3' ); ?>" x1="0" y1="0" x2="0" y2="1">
							<stop offset="0%" stop-color="#7de3b3" stop-opacity="0.45"></stop>
							<stop offset="100%" stop-color="#7de3b3" stop-opacity="0"></stop>
						</linearGradient>
					</defs>
