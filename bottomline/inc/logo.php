...
function bl_sync_logo($value, $from) {
    ...
    try {
        ...
        if ($from === 'wordpress') {
            ...
            if (function_exists('acf_flush_value_cache')) { acf_flush_value_cache('options', 'site_logo'); acf_flush_value_cache('option', 'site_logo'); }
        }
    } finally { $syncing = false; }
    return $id;
}
add_filter('acf/update_value/name=site_logo', function ($value, $post_id) {
    return in_array($post_id, ['option', 'options'], true) ? bl_sync_logo($value, 'acf') : $value;
}, 10, 2);
...
add_action('updated_option', function ($option, $old, $new) {
    if ($option === 'theme_mods_' . get_option('stylesheet') && ($old['custom_logo'] ?? 0) !== ($new['custom_logo'] ?? 0)) {
        bl_sync_logo($new['custom_logo'] ?? 0, 'wordpress');
    }
}, 10, 3);
add_action('added_option', function ($option, $value) {
    if ($option === 'theme_mods_' . get_option('stylesheet') && is_array($value) && array_key_exists('custom_logo', $value)) { bl_sync_logo($value['custom_logo'], 'wordpress'); }
}, 10, 2);
add_action('deleted_option', function ($option) {
    if ($option === 'theme_mods_' . get_option('stylesheet')) { bl_sync_logo(0, 'wordpress'); }
});
